...
//
// MIGRATION NOTE (Cloudflare Pages Functions + Firestore REST):
// dispatchTelegramEvent(arg1, event, data) keeps the exact same name and
// the exact same 3-argument shape it always had — but arg1's MEANING has
// changed. It used to be a firebase-admin Firestore `db` handle
// (getDb()); it is now the Cloudflare Pages `env` object (context.env),
// because every lib/firestore-rest.js function takes `env` as its first
// argument instead of a db handle. Every caller of dispatchTelegramEvent
// (place-order.js, submit-review.js, the two files in this module) must
// pass `context.env`, not a db object. See REPORT.md for the full
// cross-worker coordination note on this.

import { getDocs } from "./firestore-rest.js";

// Hard cap on how long we'll wait for Telegram's API before giving up on
// that one bot. Without this, a slow/hanging Telegram API left `fetch()`
// with no timeout at all — and because dispatchTelegramEvent() is
// `await`ed directly in the checkout flow (functions/api/place-order.js),
// a hung Telegram request meant a hung checkout response for the
// customer, even though the order itself had already been saved. This
// does NOT change the "never throws" contract: a timeout is caught by
// the existing try/catch below exactly like a network error would be.
const TELEGRAM_FETCH_TIMEOUT_MS = 8000;

function fetchWithTimeout(url, options, timeoutMs = TELEGRAM_FETCH_TIMEOUT_MS) {
...
// button per row.
// (Unchanged from the Vercel version — these are pure functions with no
...
function buildNewOrder(data) {
  ...
  const itemLines = items
    .map((it, i) => {
      const variant = (it.size || it.color) ? ` [${[it.size, it.color].filter(Boolean).map(esc).join("/")}]` : "";
      return `${i + 1}. ${esc(it.title)}${variant} x${it.quantity} @ ${money(it.price)}`;
    })
    .join("\n");
...
function buildNewContactTicket(data) {
  let text = `📩 <b>NEW CONTACT MESSAGE</b>\n━━━━━━━━━━━━━━━━━━\n`;
  text += `👤 Name: ${esc(data.name)}\n`;
  text += `✉️ Email: ${esc(data.email)}\n`;
  text += `🏷 Subject: ${esc(data.subject)}\n\n`;
  text += `"${esc((data.message || "").slice(0, 400))}"\n\n`;
  text += `Reply from the Admin Panel > Support Tickets.`;

  const buttons = [];
  if (data.adminUrl) buttons.push({ label: "👁 Open in Admin Panel", url: data.adminUrl });
  return { text, buttons };
}
...
function buildMessage(event, data) {
  switch (event) {
    ...
    case "new_contact_ticket": return buildNewContactTicket(data);
...
// ---------------------------------------------------------------------
// Telegram Bot API call (unchanged — plain fetch, no Firestore involved)
// ---------------------------------------------------------------------
 ...
 *
 * @param {object} env - Cloudflare Pages `context.env` (was a firebase-admin
 *   `db` handle in the Vercel version — see migration note at the top of
 *   this file).
 * @param {string} event
 * @param {object} data
 */
export async function dispatchTelegramEvent(env, event, data) {
  let results = [];
  try {
    const bots = await getDocs(env, "telegram_bots");
    const { text, buttons } = buildMessage(event, data);

    const eligibleBots = (bots || [])
      .filter((bot) => bot.active && Array.isArray(bot.events) && bot.events.includes(event) && bot.token && bot.chatId);

    // Parallel send across all eligible bots (deliberate fix for submission
    // speed — do NOT revert this to a sequential for-loop).
    results = await Promise.all(eligibleBots.map(async (bot) => {
      try {
        let r;
        ...
        return { bot: bot.name, ok: !!r.ok, error: r.ok ? null : r.description };
      } catch (err) {
        console.error(`Telegram send failed for bot "${bot.name}":`, err.message);
        return { bot: bot.name, ok: false, error: err.message };
      }
    }));
  } catch (err) {
