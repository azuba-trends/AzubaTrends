...
//
// WORKER 2 MIGRATION NOTE: copied over verbatim. Verified it has zero
// imports and zero Node-only APIs (no `crypto`, no `fs`, no Admin SDK) —
// it's pure arithmetic on plain numbers, so it runs identically on the
// Cloudflare Workers runtime with no changes required.
