...
// Hard cap on how long we'll wait for a push service (FCM/Chrome/etc) to
// accept a single notification. Previously this fetch() had no timeout,
// so one slow/unreachable push endpoint could hold its Promise open
// indefinitely inside the Promise.all() fan-outs in send-push.js and
// notify-restock.js, delaying delivery to every other device in that
// same batch. A timeout here is a plain rejected fetch — the existing
// per-subscription try/catch at each call site already handles it the
  ...
  try {
    resp = await fetch(subscription.endpoint, {
      ...
      headers: {
        ...
        // Tells the push service (FCM/Chrome) this notification matters and
        // shouldn't be delayed or deprioritized by battery-saver/Doze mode —
        // one of the factors (along with image/vibrate/renotify above and
        // genuine user engagement over time) that pushes Chrome toward
        // showing it as a floating heads-up banner instead of only landing
        // quietly in the notification tray.
        "Urgency": "high",
